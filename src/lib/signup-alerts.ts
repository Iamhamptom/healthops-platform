...
async function notifyViaNtfy(info: SignupInfo) {
  try {
    await fetch(`https://ntfy.sh/${NTFY_TOPIC}`, {
      ...
      body: `${info.name} (${info.email}) just signed up for VisioHealth OS`,
    });
  } catch {
...
async function notifyViaEmail(info: SignupInfo) {
  ...
  try {
    ...
    await sendEmail({
      to: OWNER_EMAIL,
      subject: `New Signup: ${info.name} — VisioHealth OS`,
      html: `
        <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:500px;margin:0 auto;padding:24px;">
          ...
          <p style="color:#555;font-size:13px;">— VisioHealth OS Signup Alert</p>
        </div>`,
    });
  } catch {
