...
export async function POST(request: Request) {
  const rl = rateLimitByIp(request, "public/refer", { limit: 10, windowMs: 60_000 });
  ...
  if (isDemoMode) {
    return NextResponse.json({
      referralId: "demo-ref-" + Math.random().toString(36).slice(2, 10),
      status: "pending",
...
export async function GET(request: Request) {
  ...
  if (isDemoMode) {
    return NextResponse.json({
      practice: {
        name: "Demo ENT Practice",
        type: "ent",
        primaryColor: "#D4AF37",
        secondaryColor: "#2DD4BF",
        services: ["Sinus Consultation", "FESS Surgery", "Balloon Sinuplasty", "Septoplasty", "Tonsillectomy"],
      },
    });
  }
  ...
  let services: string[] = [];
  try {
    const parsed = JSON.parse(practice.bookingServices || "[]");
    services = parsed.map((s: { name: string }) => s.name);
  } catch {}

  return NextResponse.json({ practice: { ...practice, services } });
}
