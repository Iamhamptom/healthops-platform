...
const regulations = [
  {
    ...
    color: "#10b981",
    requirements: [
      { item: "Explicit consent for health data processing", status: "compliant", products: ["VisioHealth OS"] },
      { item: "Separate marketing consent (not bundled)", status: "compliant", products: ["VisioHealth OS"] },
      { item: "POPIA consent records with audit trail", status: "compliant", products: ["VisioHealth OS"] },
      { item: "Right to withdraw consent", status: "compliant", products: ["VisioHealth OS"] },
      { item: "Information Officer registration", status: "planned", products: ["VisioHealth OS"] },
      { item: "Cross-border data transfer compliance", status: "compliant", products: ["VisioHealth OS", "Visio Health Integrator"] },
      { item: "Data retention policy enforcement", status: "compliant", products: ["VisioHealth OS"] },
      { item: "Breach notification procedures", status: "planned", products: ["VisioHealth OS"] },
    ],
  },
  {
    name: "HPCSA",
    ...
    color: "#8B5CF6",
    requirements: [
      { item: "Practitioner verification before onboarding", status: "planned", products: ["VisioHealth OS", "Placeo Health"] },
      { item: "No canvassing/touting in platform marketing", status: "compliant", products: ["Placeo Health"] },
      { item: "Patient record retention (6yr+ from dormancy)", status: "compliant", products: ["VisioHealth OS"] },
      { item: "Audit logging for all patient data access", status: "compliant", products: ["VisioHealth OS"] },
      { item: "Role-based access control", status: "compliant", products: ["VisioHealth OS"] },
      ...
      { item: "Scope of practice enforcement", status: "planned", products: ["VisioHealth OS", "VisioMed AI"] },
    ],
  },
  {
    name: "ICD-10 SA",
    ...
    color: "#3B82F6",
    requirements: [
      { item: "ICD-10 codes in invoice line items", status: "compliant", products: ["VisioHealth OS"] },
      { item: "SA Master Industry Table codes only", status: "planned", products: ["VisioHealth OS", "VisioHealth Payer Connect"] },
      { item: "Highest specificity enforcement", status: "planned", products: ["VisioHealth OS"] },
      { item: "External Cause Code pairing for T-codes", status: "planned", products: ["VisioHealth OS", "VisioHealth Payer Connect"] },
      { item: "AI-assisted ICD-10 code suggestion", status: "planned", products: ["VisioMed AI"] },
      { item: "PMB condition auto-detection", status: "planned", products: ["VisioHealth Payer Connect"] },
    ],
  },
  {
    name: "CPA",
    ...
    color: "#f59e0b",
    requirements: [
      { item: "Transparent fee disclosure (Booklet 19)", status: "compliant", products: ["VisioHealth OS"] },
      { item: "Direct marketing time restrictions", status: "planned", products: ["VisioHealth OS"] },
      { item: "Opt-out mechanism in all communications", status: "compliant", products: ["VisioHealth OS"] },
      { item: "Plain language in patient communications", status: "compliant", products: ["VisioHealth OS"] },
  ...
  },
  {
    ...
    color: "#ec4899",
    requirements: [
      { item: "Compliant claim submission format", status: "planned", products: ["VisioHealth Payer Connect"] },
      { item: "PMB (Prescribed Minimum Benefits) tracking", status: "planned", products: ["VisioHealth Payer Connect"] },
      { item: "No false coverage guarantees in marketing", status: "compliant", products: ["VisioHealth OS"] },
      { item: "Medical aid claim status transparency", status: "compliant", products: ["VisioHealth OS"] },
    ],
  },
  {
    name: "Medicines Act",
  ...
  },
  {
    ...
    color: "#0ea5e9",
    requirements: [
      { item: "Business details on website (name, address, registration)", status: "compliant", products: ["VisioHealth OS"] },
      { item: "Electronic consent legally valid", status: "compliant", products: ["VisioHealth OS", "Visio Waiting Room"] },
      { item: "SSL/TLS encryption on all data transmission", status: "compliant", products: ["VisioHealth OS"] },
      { item: "Terms & conditions accessible", status: "compliant", products: ["VisioHealth OS"] },
    ],
  },
];

const productIcons: Record<string, React.ElementType> = {
  "VisioHealth OS": HeartPulse,
  "Placeo Health": Building2,
  "Visio Health Integrator": Plug,
  "Visio Waiting Room": Clock,
  "VisioHealth Payer Connect": CreditCard,
  "VisioMed AI": Brain,
};
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          How VisioHealth meets South African healthcare regulations across every product.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-5 text-center">
          <p className="text-4xl font-bold text-emerald-700 font-serif">{complianceRate}%</p>
          <p className="text-sm text-emerald-600 mt-1">Compliance Score</p>
          <p className="text-xs text-emerald-500 mt-0.5">{compliantItems} of {totalItems} items compliant</p>
        </div>
