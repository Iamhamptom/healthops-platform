...
export default function SettingsPage() {
  ...
  return (
    <div className="p-6 max-w-2xl space-y-6">
      ...
      {/* Practice profile */}
      <form onSubmit={handleSubmit} className="rounded-xl bg-white border border-[var(--border-light)] p-6 space-y-5">
        ...
        <button
          type="submit"
          disabled={loading}
          className="flex items-center gap-2 px-6 py-2.5 bg-[var(--primary)] hover:bg-[var(--primary-dark)] rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-50"
        >
          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
          {hasPractice ? "Save Changes" : "Create Practice"}
        </button>

        {saved && <p className="text-sm text-[var(--accent)]">Settings saved successfully!</p>}
      </form>

      {/* Account info */}
      <div className="rounded-xl bg-white border border-[var(--border-light)] p-6">
        <h3 className="text-sm font-medium mb-3">Account</h3>
        <p className="text-sm text-[var(--text-secondary)]">Signed in as <span className="text-[var(--text-primary)]">{userName}</span></p>
      </div>
    </div>
  );
}
